...
- распределяет и перераспределяет нагрузку между серверами (нодами) за счёт планировщика;
...
- обновляет конфигурацию Envoy динамически (через xDS), когда меняется количество реплик или состояние подов;
