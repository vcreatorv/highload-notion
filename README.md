...
На L4 используется Google Cloud Load Balancer. Он принимает входящий трафик на Anycast-IP, проверяет состояние бэкендов и распределяет соединения между нодами Kubernetes-кластера.

Дальше трафик попадает в Kubernetes Service, который выступает локальным L4-балансировщиком внутри кластера и раскидывает TCP/UDP-соединения по нодам, где запущены ingress-поды с Envoy. Для этого используется kube-proxy, опирающийся на актуальное состояние нод и подов из Kubernetes API.
...
При загрузке файла мы в одной транзакции PostgreSQL сохраняем метаданные записи со статусом pending и одновременно вставляем событие «загрузить в S3» в таблицу outbox_events. Фоновый worker постоянно опрашивает эту таблицу, берёт новые события, загружает файл из временного хранилища в S3, получает постоянный URL и в новой транзакции обновляет статус записи на uploaded и помечает событие как обработанное. Если worker падает — после рестарта он продолжает с того же места. Таким образом достигается гарантированная доставка файла в S3 без риска потерять данные
...
- 1 ядро CPU = 500 RPS (легкая бизнес-логика - далее "Л")
- 1 ядро CPU = 100 RPS (средняя бизнес-логика - далее "С")
- 1 ядро CPU = 50 RPS (тяжелая бизнес-логика - далее "Т")

| Сервис                  | Пиковый RPS       | Тип нагрузки | CPU (vCPU) | RAM (ГБ) | Disk (суммарно по ЦОДам) |
|-------------------------|-------------------:|--------------|------------|----------|--------------------------|
| Envoy + API GW          | 44 364            | –            | 180        | 24       | –                        |
| Page Service            | 25 810            | Т            | 120        | 40       | –                        |
| Search Service          | 752               | Т            | 12         | 8        | –                        |
| ACL Service             | 70                | Л            | 4          | 2        | –                        |
| Share Service           | 46                | Л            | 4          | 2        | –                        |
| Attach + Media          | 14 178            | С            | 185        | 90       | –                        |
| Views Service           | 3 472             | Л            | 15         | 6        | –                        |
| Auth Service            | 300               | С            | 6          | 4        | –                        |
| User Service            | 24                | Л            | 6          | 2        | –                        |
| Activity Service        | ~3 000            | С            | 20         | 4        | –                        |
| Kafka consumer          | 800               | С            | 8          | 3        | –                        |
| Redis                   | 500               | –            | 10         | 20       | 1 ТБ                     |
| PostgreSQL              | 5000              | –            | 100        | 40       | 350 ТБ                   |
| ElasticSearch           | 1500              | –            | 40         | 20       | 700 ТБ                   |
| AWS S3                  | 7000              | –            | 70         | 35       | 4 ПБ                     |
| ClickHouse              | 5000              | –            | 50         | 25       | 10 ТБ                    |
...
### Расчет количества и комплектации серверов

У нас 16 ЦОДов, размещение равномерное, отказоустойчивость — минимум 3 реплики на разные ЦОДы.  
Все Go-микросервисы + Kafka consumers запускаются в Kubernetes на общих нодах типа **Kube node**.  
Отдельные физические сервера выделяем только под тяжёлые stateful-системы.

#### Итоговые требования по ресурсам (суммарно по всем 16 ЦОДам)

| Категория                        | vCPU   | RAM, ГБ |
|----------------------------------|--------|---------|
| Все Go-микросервисы + Kafka consumers | 180 + 120 + 12 + 4 + 4 + 185 + 15 + 6 + 6 + 20 + 8 = **560 vCPU** | 24 + 40 + 8 + 2 + 2 + 90 + 6 + 4 + 2 + 4 + 3 = **185 ГБ** |
| Redis                            | 10     | 20      |
| PostgreSQL                       | 100    | 40      |
| ElasticSearch                    | 40     | 20      |
| ClickHouse                       | 50     | 25      |
...
- vCPU: 560 (Go + consumers) + 10 (Redis) + 100 (PG) + 40 (ES) + 50 (CH) = 920 vCPU
- RAM: 185 (Go + consumers) + 20 (Redis) + 40 (PG) + 20 (ES) + 25 (CH) = 290 ГБ

#### Конфигурации серверов и количество

| Сервис                     | Хостинг | Конфигурация                                                                                  | Cores   | Count | Покупка       | Аренда/Амортизация          |
|----------------------------|---------|------------------------------------------------------------------------------------------------|---------|-------|---------------|-----------------------------|
| Kube node                  | own     | 2U/2×AMD EPYC 7543P (32×2)/512 ГБ RAM/2×NVMe 1.92 ТБ RAID1/2×25 Gbit/s                         | 64      | 44    | $16 800       | $1 350 /мес                 |
| Redis                      | own     | 1U/1×AMD EPYC 7402P (24 ядра)/256 ГБ RAM/2×NVMe 960 ГБ RAID1/2×25 Gbit/s                        | 24      | 7     | $8 200        | $680 /мес                   |
| PostgreSQL                 | own     | 2U/2×AMD EPYC 7642 (48×2)/768 ГБ RAM/8×NVMe 7.68 ТБ RAID10/2×100 Gbit/s                        | 96      | 24    | $48 000       | $3 900 /мес                 |
| ElasticSearch data-node    | own     | 2U/2×AMD EPYC 7452 (32×2)/512 ГБ RAM/12×NVMe 15.36 ТБ/2×100 Gbit/s                             | 64      | 18    | $52 000       | $4 200 /мес                 |
| ElasticSearch master-only  | own     | 1U/1×AMD EPYC 7313P (16 ядер)/128 ГБ RAM/2×NVMe 960 ГБ/2×25 Gbit/s                            | 16      | 3     | $7 500        | $620 /мес                   |
| ClickHouse                 | own     | 2U/2×AMD EPYC 7543 (32×2)/512 ГБ RAM/8×NVMe 15.36 ТБ RAID10/2×100 Gbit/s                       | 64      | 12    | $44 000       | $3 600 /мес                 |
| AWS S3                     | cloud   | Managed object storage (4 ПБ аттачей + медиа)                                                 | –       | –     | –             | ~$85 000 /мес (pay-as-you-go) |

Итого own-серверы (аренда/амортизация): $227 120 /мес
AWS S3: $85 000 /мес

Общая стоимость аренды/амортизации в месяц:
$227 120 + $85 000 = $312 120 /мес
(≈ $3.745 млн/год)
