...
Для распределения нагрузки внутри одного дата-центра будем использовать алгоритм Weighted Response Time. Балансировщик отслеживает среднее время отклика серверов и количество активных соединений, и перенаправляет новые запросы к тем машинам, которые отвечают быстрее. Такой подход обеспечивает равномерную загрузку и минимизирует задержки.
